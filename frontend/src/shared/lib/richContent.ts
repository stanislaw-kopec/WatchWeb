...
export function normalizeRichContentForEditor(content: string) {
  if (!content.trim()) {
    return '<p><br></p>'
  }
...
export function richContentToText(content: string) {
  const document = new DOMParser().parseFromString(sanitizeRichContent(content), 'text/html')
  return (document.body.textContent ?? '').replace(/\s+/g, ' ').trim()
}

export function hasMeaningfulRichContent(content: string) {
  const document = new DOMParser().parseFromString(sanitizeRichContent(content), 'text/html')
...
function isSafeImageSource(value: string) {
  return /^https:\/\//i.test(value) || /^\/api\/files\/(article-images|post-images)\/[a-z0-9._-]+$/i.test(value)
}
