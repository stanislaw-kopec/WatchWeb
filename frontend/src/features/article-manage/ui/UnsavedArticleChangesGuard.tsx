...
export function UnsavedArticleChangesGuard({ when, onSave, saveLabel = 'Zapisz wersję roboczą' }: UnsavedArticleChangesGuardProps) {
  ...
  async function saveAndLeave() {
    ...
    try {
      ...
      setSaveError(error instanceof Error ? error.message : 'Nie udało się zapisać artykułu.')
      setIsSaving(false)
    }
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/45 p-4">
      <div
        aria-labelledby="unsaved-article-title"
        aria-modal="true"
        ...
        tabIndex={-1}
      >
        <h2 className="text-xl font-semibold text-foreground" id="unsaved-article-title">Niezapisane zmiany</h2>
        <p className="mt-2 text-sm leading-6 text-muted-foreground">
          Artykuł zawiera niezapisane zmiany. Czy chcesz je zapisać przed opuszczeniem edytora?
        </p>
