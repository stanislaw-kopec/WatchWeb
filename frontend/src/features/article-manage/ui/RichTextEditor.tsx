...
import { uploadArticleContentImage } from '@/entities/article/api/articleApi'
...
export function RichTextEditor({ value, onChange, onBlur, disabled, invalid }: RichTextEditorProps) {
  ...
  useEffect(() => {
    ...
    const normalizedValue = normalizeArticleContentForEditor(value)
  ...
  async function handleImageChange(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const image = await uploadArticleContentImage(file)
  ...
  return (
    <div className={cn('overflow-hidden rounded-md border bg-card', invalid ? 'border-destructive' : 'border-input')}>
      <div className="flex flex-wrap gap-1 border-b border-border bg-secondary/45 p-2" role="toolbar" aria-label="Formatowanie artykułu">
      ...
      <div
        ...
        aria-multiline="true"
        className="article-rich-content min-h-96 px-4 py-3 outline-none focus:ring-2 focus:ring-inset focus:ring-ring"
