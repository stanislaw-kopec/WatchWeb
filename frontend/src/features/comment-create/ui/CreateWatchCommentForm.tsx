...
import { createWatchComment } from '@/entities/comment/api/commentApi'
...
const createWatchCommentSchema = z.object({
  content: z
    .string()
    .trim()
...
type CreateWatchCommentFormValues = z.infer<typeof createWatchCommentSchema>

type CreateWatchCommentFormProps = {
  watchId: string
...
export function CreateWatchCommentForm({
  watchId,
  parentId = null,
  ...
  placeholder = parentId
    ...
    : 'Dodaj pytanie, obserwację albo doświadczenie z tym modelem.',
  onCancel,
  onSuccess,
}: CreateWatchCommentFormProps) {
  const queryClient = useQueryClient()
  const { isAuthenticated } = useAuthSession()
  const form = useForm<CreateWatchCommentFormValues>({
    resolver: zodResolver(createWatchCommentSchema),
    defaultValues: {
      content: '',
    },
  })
  ...
  const createCommentMutation = useMutation({
    mutationFn: (values: CreateWatchCommentFormValues) =>
      createWatchComment(watchId, {
        parentId,
        content: values.content.trim(),
      }),
    onSuccess: async () => {
      form.reset({ content: '' })
      await queryClient.invalidateQueries({ queryKey: ['watch-comments', watchId] })
      onSuccess?.()
    },
  })

  if (!isAuthenticated) {
    return renderUnauthenticatedState(watchId, variant)
  }

  function handleSubmit(values: CreateWatchCommentFormValues) {
...
function renderUnauthenticatedState(watchId: string, variant: 'card' | 'inline') {
  const content = (
